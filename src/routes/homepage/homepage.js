...
export default class HomePage extends React.Component {
	...
	updateCaloriesOfTheMonth = () => {
		...
		const meals = allMeals
			.filter((obj) => format(obj.dateofmeal) === month)
			.map((obj) => Number(obj.alldaycalories));
		if (meals) {
	...
	updateMeals = () => {
		...
		MealApiServices.getMealsByUser(userId).then((array) => this.setState({ allMeals: array })).then(() => {
